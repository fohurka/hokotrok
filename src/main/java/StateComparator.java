 ...
 *     or as an object key in an ID-keyed object like bank.accounts or lane.vehicles)
...
public class StateComparator {
    ...
    static class JNull    extends Node { static final JNull I = new JNull(); }
    static class JBool    extends Node { final boolean v; JBool(boolean v)  { this.v = v; } }
    static class JNum     extends Node { final double  v; JNum(double v)    { this.v = v; } }
    static class JStr     extends Node { final String  v; JStr(String v)    { this.v = v; } }
    static class JArr     extends Node { final List<Node> elems;
        JArr(List<Node> e) { elems = e; } }
    ...
    static class JObj     extends Node {
        ...
        JObj(List<String> ko, Map<String, Node> f) { keyOrder = ko; fields = f; }
    }
     ...
     * @return true if the files are structurally equivalent under the ID-mapping rules
     */
    ...
    private void collectIds(Node node) {
        if (node instanceof JObj) {
            ...
            for (Node e : ((JArr) node).elems) collectIds(e);
        }
    }
    ...
    private boolean compareNodes(Node n1, Node n2, String key) {
        if (n1 instanceof JNull && n2 instanceof JNull) return true;
        ...
        if (n1 instanceof JObj  && n2 instanceof JObj)
            return compareObjects((JObj) n1, (JObj) n2);
    ...
    private boolean compareObjects(JObj o1, JObj o2) {
        if (o1.keyOrder.size() != o2.keyOrder.size()) return false;

        // Special case: objects whose keys are IDs (e.g. bank.accounts, lane.vehicles).
        // Detected when every key in o1 is a known ID.
        if (!o1.keyOrder.isEmpty() && o1.keyOrder.stream().allMatch(knownIds::contains)) {
            return compareIdKeyedObjects(o1, o2);
        }

        // Normal case: keys must match exactly.
        if (!o1.fields.keySet().equals(o2.fields.keySet())) return false;
        for (String key : o1.keyOrder) {
            if (!compareNodes(o1.fields.get(key), o2.fields.get(key), key)) return false;
        }
        return true;
    }
    ...
    private boolean matchIdKeyedEntries(JObj o1, JObj o2,
        ...
        for (int j = 0; j < keys2.size(); j++) {
            ...
            if (compareStrings(k1, k2)
                ...
                used[j] = true;
                if (matchIdKeyedEntries(o1, o2, keys1, keys2, idx + 1, used)) return true;
                used[j] = false;
            }
            fwdMap = sf;
            revMap = sr;
    ...
    /** "recoveryQueue" is the only strictly ordered array. Everything else is unordered. */
    private boolean compareArrays(JArr a1, JArr a2, String key) {
        if (a1.elems.size() != a2.elems.size()) return false;
        if ("recoveryQueue".equals(key)) {
            for (int i = 0; i < a1.elems.size(); i++) {
                if (!compareNodes(a1.elems.get(i), a2.elems.get(i), null)) return false;
            }
            return true;
        }
    ...
    private boolean matchUnordered(List<Node> l1, List<Node> l2, int idx, boolean[] used) {
        ...
        for (int j = 0; j < l2.size(); j++) {
            ...
            if (compareNodes(n1, l2.get(j), null)) {
                used[j] = true;
                if (matchUnordered(l1, l2, idx + 1, used)) return true;
                used[j] = false;
            }
            fwdMap = sf;
            revMap = sr;
    ...
    private static class JsonParser {
        ...
        private JObj parseObject() {
            ...
            skipWs();
            if (peek() == '}') { pos++; return new JObj(keyOrder, fields); }
            while (true) {
                skipWs();
        ...
        private JArr parseArray() {
            ...
            skipWs();
            if (peek() == ']') { pos++; return new JArr(elems); }
            while (true) {
                skipWs();
        ...
        private JBool parseBool() {
            if (s.startsWith("true", pos))  { pos += 4; return new JBool(true);  }
            pos += 5; return new JBool(false);
        }

        private JNull parseNull() { pos += 4; return JNull.I; }
